...
PIP.func<-function(likeli,model.space,p){
  ...
  l=model.space
  model.space=matrix(0,nrow=length(l),ncol=p)
  for (i in 1: length(l)){
    ind=as.numeric(unlist(strsplit(l[i],split=",")))
    if (length(ind)>0) model.space[i,ind]=1
  }
...
index.fun<-function(x,p){
  l=apply(x,MAR=1,FUN=function(x){
          sort(x);
          paste0(x,collapse = ",")
          })
  return(l)
...
MCS_modified<-function(z,ld.matrix,Max.Model.Dim=1e+4,lambda,
  ...
  out=list(result.B.list,C.list=NULL,result.prob=NULL,conditional.S.list)
  return(out)
}
